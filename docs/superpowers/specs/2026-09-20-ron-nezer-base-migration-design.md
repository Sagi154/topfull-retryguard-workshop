...
- **Loadgen layer (d) specifics** — **shape resolved 2026-09-21**, see [2026-09-21-loadgen-shape-ron-migration-design.md](2026-09-21-loadgen-shape-ron-migration-design.md): adopt Ron's independent-per-tag-swarm shape (not either of his two launcher scripts' literal numbers — a fresh audit found his own history is mid-tuning, not a settled recipe) and his `@task()` weight remix (adopted for fidelity though inert under that shape); `experiments/loadgen/online_boutique_create_v2.sh` is the implementation vehicle. **Still deferred**: actual loadgen numbers (`GETPRODUCT`/etc. per scenario) and the S1–S6 methodology rework below — both wait on a fresh calibration pass under the Ron-config regime, bundled together per that doc's §3. This also **invalidates the capacity numbers** in [2026-09-20-loadgen-getcart-postcart-and-load-scaling-design.md](2026-09-20-loadgen-getcart-postcart-and-load-scaling-design.md), which were calibrated against the paper-quota regime (`capacity_frozen.json`) — that doc's §4 numbers need to be redone once this migration's CPU numbers are final, not reused as-is.
