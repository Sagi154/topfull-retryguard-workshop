...
Checked directly against Task 8's post-migration constrained holds (legacy launcher, superseded as freezes, but usable as sizing evidence for the *target* tags that did spawn under integer `count/RATE`): 50m frontend `mu_sat`≈13 rps, checkout≈74, payment≈12, productcatalog≈81. Pre-migration "far above scenario default" counts (`postcheckout: 500`, `getproduct: 800`) overshoot those ceilings by 7–40× under Ron-config headroom and peg helper services. **Task 8a adopts ~3× those measured goodputs** (frontend ~4×, storefront-shaped) with light non-target tags at 10 and `spawn_rate: 50` (Ron's divisor). Those numbers are the battery's starting loads for Task 8b — adjust further only if a hold fails to saturate or still compounds.
...
1. **Frontend, 50m, HPA pinned off for the hold.** Single-replica isolation — the calibration is measuring one replica's ceiling; that's a different question from "how does frontend behave with HPA on," which is answered separately by just running real scenarios once frontend's normal cap (1150m) and HPA are both live. Starting load: `getproduct: 10, postcheckout: 5, getcart: 10, postcart: 5, emptycart: 20, spawn_rate: 50` (~4× measured 50m `mu_sat` ≈13 rps, storefront-shaped; not the S2 create.sh mix).
2. **Checkoutservice, 50m, paymentservice at its real 155m** (not artificially bumped). Starting load: `postcheckout: 220` (~3× measured 50m `mu_sat` ≈74 rps from the post-migration constrained hold), everything else light at 10, `spawn_rate: 50` (Ron's divisor). Do **not** reuse the pre-migration `postcheckout: 500` — it pegs helper services under Ron-config headroom.
3. **Paymentservice, 50m, checkoutservice at its real 615m.** Starting load: `postcheckout: 40` (~3× measured 50m `mu_sat` ≈12 rps), light others at 10, `spawn_rate: 50`. The old `500` compound-bottlenecked checkout at its real 615m.
4. **Productcatalogservice, 50m, HPA pinned off for this specific hold** (even though decision 6 enables it generally — same single-replica-isolation reasoning as frontend). Starting load: `getproduct: 250` (~3× measured 50m `mu_sat` ≈81 rps), light others at 10, `spawn_rate: 50`. The old `800` took down frontend and recommendations.
5. **Bottleneck reference load, unconstrained (`scale_constraints: []`).** Starting candidate (decision 8's shared-first attempt): `getproduct: 250, postcheckout: 220, getcart/postcart/emptycart: 10, spawn_rate: 50` — the two heavy tags from runs 2–4, cart family light. Check: does everything *other* than checkout/productcatalog/payment stay healthy at this load with nothing capped? If yes, this one recipe becomes the shared reference for S3/S4A/S4B. If something else gets pulled into overload, fall back to three separate reference loads instead (decision 8). The old combined `800+500` recipe already failed that test under Ron-config (frontend fail 0.66, recommendations pegged) and must not be re-used as the probe.
...
**Why that's a real problem, not a formatting nit:** the legacy launcher differs from `online_boutique_create_v2.sh` in two ways that change the load the YAML claims to specify:

1. **Bash integer `count / RATE`.** `RATE` is a divisor in both Ron's launchers and (as of 2026-09-22) v2 — Locust `-r` is `count / RATE`, and a bigger `RATE` is a slower ramp. Ron and v2 compute that with `awk`, so the result is a float (`postcheckout: 20`, `spawn_rate: 90` → `-r ≈ 0.22`, and the tag still reaches 20 users). The legacy script uses bash `$((count / RATE))`, which truncates. On the counts this project uses that often yields `-r 0` (`postcheckout: 10`, `spawn_rate: 45`) or a 1 user/sec ramp, so the tag never reaches the YAML population during a 300–900s hold. Steady offered load, once users exist, is still ~1 request/sec per user (`constant_throughput(1)`); `-r` only controls the ramp. v2 does **not** pass `spawn_rate` through as users/sec.
...
| 16e | No YAML schema change and no per-tag `RATE_*` override support added at this time — a bare `spawn_rate` continues to map to v2's shared `RATE` divisor (`-r = count / RATE` via awk), applied identically to all 5 independent tag processes. If a future session wants genuinely different per-tag ramp divisors, that is a new, separately-decided schema addition, not implied by this amendment. The script already accepts optional `RATE_*` env vars; the runner does not export them. |
