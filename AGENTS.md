...
> **Next session — do this first:** TopFull-on-e2-16 is confirmed. Open questions: (1) RetryGuard still does not disable on flat S2 because admission absorbs overload before mesh inbound — try S3/S4, tighter `per_try_timeout_ms`, or a different signal; (2) S2 retries stay near-zero when TopFull is active — tighten timeout or use a constrained-backend scenario if a retry storm is required. Share [MENTOR-UPDATE.md](Guides%20and%20Info/mentor-update/MENTOR-UPDATE.md) with mentors (note e2-8 `campaign_48` S1–S4 had inert TopFull; e2-16 S1/S2 checkpoints now have live TopFull). Then write Phase 7 eval on **`experiments/results/campaign_48/`** once mentors say what's sufficient. Do not overwrite `august_38/` or the new run22/run21/run10 folders.
>
...
>
> **S6 YAMLs are already at run4.** S1 baseline YAML is **run23**; S2 baseline **run22**; S2 RetryGuard **run11**. Other S1–S5 YAMLs still sit on the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them.
>
