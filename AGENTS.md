...
> **Next session — do this first:** TopFull-on-e2-16 is confirmed; TopFull-off control shows retries return but modestly. Open questions: (1) RetryGuard still does not disable on flat S2 with TopFull on — try S3/S4, tighter `per_try_timeout_ms`, or a different signal; (2) if a large retry storm is required on e2-16, tighten timeout further or constrain a backend. Share [MENTOR-UPDATE.md](Guides%20and%20Info/mentor-update/MENTOR-UPDATE.md) with mentors (note e2-8 `campaign_48` S1–S4 had inert TopFull; e2-16 S1/S2 checkpoints now have live TopFull). Then write Phase 7 eval on **`experiments/results/campaign_48/`** once mentors say what's sufficient. Do not overwrite `august_38/` or run22/run21/run10 / `baseline_no_topfull_*_run1`.
>
> **§7b / RetryGuard follow-up:** On e2-16 S2 RG run10, still zero disables (same as run9, different cause: TopFull upstream of mesh). Decide Threshold / `interval_samples` / `per_try_timeout_ms` / S3–S4 path before claiming controller action. Do not overwrite run19/run9/run10.
>
> **S6 YAMLs are already at run4.** S1 baseline YAML is **run23**; S2 baseline **run23**; S2 RetryGuard **run12**; no-TopFull control YAML is **run2**. Other S1–S5 YAMLs still sit on the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them.
>
