...
>
> **Verification run for §7b:** Run one S2 baseline with the perTryTimeout fix applied. Confirm `envoy_retry_collector` shows `retry > 0` on `service_edges.csv` and `topfull_throttle.csv` fresh `threshold < 10000` (if §7a is also applied). Remember to bump `run_number` / `log_folder` first (S2 baseline YAML is currently at **run18**).
>
...
**Before repeating a scenario:** bump `run_number` and change `log_folder` in the YAML, or the new run will overwrite the folder **on master**. Completed campaign slots are run4–6 (S1–S4), run3–5 (S5), run1–3 (S6). Post-campaign checkpoint: S1 baseline **run20** (YAML now **run21**), S2 baseline **run17** (YAML now **run18**). Other S1–S5 YAMLs still point at the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them. **S6 YAMLs still point at completed run3 — bump to run4 before any extra S6 run.** Never reuse August `run1–3` (S1–S4) or S5 `run1–2`.
