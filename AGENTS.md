...
>
> **S6 YAMLs are already at run4.** S1 baseline YAML is **run23**; S2 baseline **run22**; S2 RetryGuard **run11**; no-TopFull control YAML is **run2**. Other S1–S5 YAMLs still sit on the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them.
>
...
**Before repeating a scenario:** bump `run_number` and change `log_folder` in the YAML, or the new run will overwrite the folder **on master**. Completed campaign slots are run4–6 (S1–S4), run3–5 (S5), run1–3 (S6). Post-campaign checkpoint: S1 baseline **run22** (YAML now **run23**), S2 baseline **run21** (YAML **run22**), S2 RetryGuard **run10** (YAML **run11**); earlier e2-8 / §7b folders remain historical. S6 YAMLs are at **run4**. Other S1–S5 YAMLs still point at the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them. Never reuse August `run1–3` (S1–S4) or S5 `run1–2`.
