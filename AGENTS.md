...
## 4. Current status (as of 2026-09-15)
...
> **Next session — do this first:** S3 on e2-16 showed RetryGuard action (disable/re-enable at checkout). Open questions: (1) RetryGuard still does not disable on flat S2 with TopFull on — S4 may clarify topology sensitivity; (2) if a large retry storm is required on e2-16 unconstrained paths, tighten timeout further or use TopFull-off control. Share [MENTOR-UPDATE.md](Guides%20and%20Info/mentor-update/MENTOR-UPDATE.md) with mentors (note e2-8 `campaign_48` S1–S4 had inert TopFull; e2-16 S1/S2/S3 checkpoints now have live TopFull + S3 RG toggles). Then write Phase 7 eval on **`experiments/results/campaign_48/`** once mentors say what's sufficient. Do not overwrite `august_38/` or run22/run21/run10 / S3 run8/run7 / `baseline_no_topfull_*_run1`.
>
> **§7b / RetryGuard follow-up:** On e2-16 S2 RG run10/run11, still zero disables (TopFull upstream of mesh). S3 run7 **did** disable at checkout — controller path works when the bottleneck is a constrained backend. Decide whether to continue with S4 or retune S2. Do not overwrite run19/run9/run10/run11 or S3 run8/run7.
>
> **S6 YAMLs are already at run4.** S1 baseline YAML is **run23**; S2 baseline **run23**; S2 RetryGuard **run12**; S3 baseline **run9**; S3 RetryGuard **run8**; no-TopFull control YAML is **run2**. Other S1/S4/S5 YAMLs still sit on the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them.
>
...
**Before repeating a scenario:** bump `run_number` and change `log_folder` in the YAML, or the new run will overwrite the folder **on master**. Completed campaign slots are run4–6 (S1–S4), run3–5 (S5), run1–3 (S6). Post-campaign checkpoints: S1 baseline **run22** (YAML now **run23**), S2 baseline **run22** (YAML **run23**), S2 RetryGuard **run11** (YAML **run12**); earlier e2-8 / §7b folders remain historical. S6 YAMLs are at **run4**. Other S1–S5 YAMLs still point at the next free campaign-style slot (run7 / S5 run6) unless a later session bumped them. Never reuse August `run1–3` (S1–S4) or S5 `run1–2`.
