...
**Latency columns (`rq_time_sum_ms` / `rq_time_count`, added 2026-09-16; inbound also `rq_time_buckets`):** the same collector scrapes Envoy's `downstream_rq_time` (inbound) and `upstream_rq_time` (outbound) histograms. Inbound is **live-verified** on this cluster; outbound `upstream_rq_time` is excluded by Istio's default minimal proxy-stats (structural zero on `service_edges.csv` until a deliberate `proxyStatsMatcher` change). `Δrq_time_sum_ms / Δrq_time_count` between polls is a per-service mean sojourn time — this service's own latency, not end-to-end/multi-hop. Inbound `rq_time_buckets` (JSON of cumulative `le` → count) lets `estimate_service_mu.py` also report a per-tick P50. Feeds the corrected `mu_hat_w = lambda + 1/W` estimator — see `docs/superpowers/specs/2026-09-16-rho-estimator-correction.md`. Runs pulled before 2026-09-16 lack these columns.
