...
- `experiments/run_scenario.py`'s `_launch_locust()` now dual-exports `EMPTYCART` alongside the legacy `CART`, so `experiments/loadgen/online_boutique_create_v2.sh` (already independently built, before this audit, to fix our own `count/RATE` and dead-env-var bugs) can read it correctly while old scenario YAMLs keep working unchanged.
