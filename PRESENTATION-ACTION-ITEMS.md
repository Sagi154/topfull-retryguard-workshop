...
- [ ] **Use `Online-Boutique-architecture.png`** on this slide to show the service call graph (Frontend → Checkout → leaf services → Redis cache). Keep it as a supporting illustration; the bullet points above must stand on their own.
...
- [ ] **Use `Online-Boutique-architecture.png`** on this slide — annotate or highlight the three positions directly on the diagram (e.g., circle ProductCatalog for position 1, Checkout for position 2, Email/Payment for position 3).
